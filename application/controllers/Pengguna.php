...
class Pengguna extends CI_Controller
{
    ...
    public function aksi_masuk_parkir()
    {
        ...
        $isAlreadyParked = $this->m_model->cek_kendaraan_parkir($id_pengguna_login);
        ...
        if (!$isAlreadyParked) {
            $kode = $this->generateRandomCode();
            $merk = $this->input->post('merk');
            $jenis_kendaraan = strtolower($this->input->post('jenis'));

            date_default_timezone_set('Asia/Jakarta');
            $jam_masuk = date('H:i:s');

            // Tentukan nominal sesuai dengan jenis kendaraan
            if ($jenis_kendaraan == 'motor') {
                $nominal_masuk = 2000;
            } elseif ($jenis_kendaraan == 'mobil' || $jenis_kendaraan == 'truk' || $jenis_kendaraan == 'bus') {
                $nominal_masuk = 4000;
            } else {

                exit('Jenis kendaraan tidak valid.');
            }
            ...
            // Tambahkan pemberitahuan berhasil masuk parkir ke dalam session flash data
            $this->session->set_flashdata('success_message', 'Kendaraan berhasil masuk parkir.');
    ...
    public function aksi_keluar()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($nominal_bayar >= $data_kendaraan['nominal_bayar']) {
                ...
                $this->session->set_flashdata('berhasil_bayar', 'Berhasil melakukan pembayaran');

                redirect(base_url('pengguna/data_kendaraan'));
            } else {
                $this->session->set_flashdata('error_pembayaran', ' Masukkan nominal yang sesuai');
                redirect(base_url('pengguna/pembayaran'));
            }
        }
    }
